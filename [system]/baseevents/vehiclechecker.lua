...
Citizen.CreateThread(function()
    while true do
        ...
        if not isInVehicle and not IsPlayerDead(PlayerId()) and not LocalPlayer.state.is_dead then
        ...
        elseif isInVehicle then
            if not IsPedInAnyVehicle(ped, false) or IsPlayerDead(PlayerId()) or LocalPlayer.state.is_dead then
                -- bye, vehicle
                local model = GetEntityModel(currentVehicle)
                local name = GetDisplayNameFromVehicleModel(model)
                local netId = VehToNet(currentVehicle)
